...
| `participant_privacy_requests` | Minimal accountability evidence | RETAIN WITH REASON | Stores status, timestamps and deletion categories only; no content or free-text reason. Organisation-scoped account deletion clears the identity link on the current and any earlier lifecycle records before the participant profile is removed. |
...
The released participant app already calls `POST /api/v1/participant/privacy/deletion-requests` with `mode_preference=delete`, `scope=account` and `confirmed=true`. The endpoint preserves that contract, so deletion after submissions is delivered by the backend and does not require a new iOS build.
