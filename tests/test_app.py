...
from app.entra import EntraAuthError, role_from_claims, safe_role_assignment, validate_claims
...
def token_generator(*values: str):
...
def test_create_workflows_reject_blank_and_malformed_input():
    with client:
        auth()

        blank_project = post_with_csrf('/projects', data={'title': '', 'code': '', 'description': '', 'status_value': 'live'}, follow_redirects=False)
        assert blank_project.status_code == 400

        project = post_with_csrf('/projects', data={'title': 'Validation Project', 'code': 'VAL-100', 'description': 'Test', 'status_value': 'live'}, follow_redirects=False)
        assert project.status_code == 303
        project_id = int(project.headers['location'].rsplit('/', 1)[-1])

        blank_study = post_with_csrf(f'/projects/{project_id}/studies', data={'title': '', 'code': '', 'description': '', 'methodology': 'diary', 'status_value': 'recruiting'}, follow_redirects=False)
        assert blank_study.status_code == 400

        bad_participant = post_with_csrf('/participants', data={'reference': 'P-BAD', 'name': 'Bad Email', 'email': 'bad-email', 'phone': '', 'status_value': 'prospective', 'consent_status': 'pending', 'communication_preference': 'email', 'tags': '', 'notes': ''}, follow_redirects=False)
...
def test_participant_enrolment_activity_and_invitation():
    with client:
        ...
        assert 'token=[REDACTED]' in outbox.text
...
def test_participant_accepts_invitation(monkeypatch):
    from app.models import ConsentEvidence
    invitation_token = f"join-token-{uuid4().hex}"
    monkeypatch.setattr(
        'app.main.new_token',
        token_generator(invitation_token, f"session-1-{uuid4().hex}", f"session-2-{uuid4().hex}"),
    )
    with client:
        auth()
        ...
        page = client.get(f'/join-study?token={invitation_token}')
        ...
        with SessionLocal() as db:
            evidence = db.scalar(select(ConsentEvidence).where(ConsentEvidence.participant_id == participant_id).order_by(ConsentEvidence.id.desc()))
            assert evidence is not None
            assert evidence.consent_wording_hash


def test_password_reset_token_is_exchanged_and_replay_is_blocked(monkeypatch):
    token = f"reset-token-{uuid4().hex}"
    monkeypatch.setattr('app.main.new_token', token_generator(token, f"reset-session-{uuid4().hex}"))
    with client:
...
def test_participant_invitation_token_is_exchanged_and_replay_is_blocked(monkeypatch):
    token = f"participant-token-{uuid4().hex}"
    monkeypatch.setattr('app.main.new_token', token_generator(token, f"participant-session-{uuid4().hex}"))
    with client:
        client.cookies.clear()
        auth()
...
def test_researcher_invitation_token_is_exchanged_and_replay_is_blocked(monkeypatch):
    token = f"researcher-token-{uuid4().hex}"
    monkeypatch.setattr('app.main.new_token', token_generator(token, f"researcher-session-{uuid4().hex}"))
    with client:
        client.cookies.clear()
        auth()
...
def test_participant_portal_draft_submit_and_message(monkeypatch):
    from app.db import SessionLocal
    from app.models import Activity, ActivityResponse, ParticipantMessage
    token = f"portal-token-{uuid4().hex}"
    monkeypatch.setattr(
        'app.main.new_token',
        token_generator(token, f"portal-session-1-{uuid4().hex}", f"portal-session-2-{uuid4().hex}"),
    )
    from sqlalchemy import select
    with client:
        auth()
        ...
        draft=post_with_csrf(f'/participant-portal/activity/{activity_id}', data={'action':'draft','answer':'draft answer','hashtags':'  Alpha,  beta,alpha  '}, follow_redirects=False)
        assert draft.status_code==303
        submit=post_with_csrf(f'/participant-portal/activity/{activity_id}', data={'action':'submit','answer':'final answer','hashtags':'Gamma, delta, gamma'}, follow_redirects=False)
        assert submit.status_code==303
        ...
        with SessionLocal() as db:
            ...
            assert response.hashtags == ['alpha', 'beta', 'gamma', 'delta']
            assert db.scalar(select(ParticipantMessage).where(ParticipantMessage.participant_id==participant_id)) is not None
        client.cookies.clear()


def test_participant_portal_hashtags_can_be_cleared_and_preserved_on_omission(monkeypatch):
    from app.db import SessionLocal
    ...
    monkeypatch.setattr(
        'app.main.new_token',
        token_generator(token, f"portal-session-3-{uuid4().hex}", f"portal-session-4-{uuid4().hex}"),
    )

    with client:
        auth()
        p = post_with_csrf('/participants', data={'reference':'P-304','name':'Portal User 2','email':'portal2@example.org','phone':'','status_value':'prospective','consent_status':'pending','communication_preference':'email','tags':'','notes':''}, follow_redirects=False)
        participant_id = int(p.headers['location'].rsplit('/', 1)[-1])
        with SessionLocal() as db:
            first_activity = db.scalar(select(Activity).order_by(Activity.id.asc()))
            study_id = first_activity.study_id
        ...
        post_with_csrf(f'/participant-portal/activity/{activity_id}', data={'action':'draft','answer':'first draft','hashtags':'Alpha, Beta'}, follow_redirects=False)
        post_with_csrf(f'/participant-portal/activity/{activity_id}', data={'action':'draft','answer':'second draft','hashtags':''}, follow_redirects=False)
        post_with_csrf(f'/participant-portal/activity/{activity_id}', data={'action':'draft','answer':'third draft'}, follow_redirects=False)

        with SessionLocal() as db:
            response = db.scalar(select(ActivityResponse).where(ActivityResponse.participant_id==participant_id, ActivityResponse.activity_id==activity_id))
            assert response is not None
            assert response.hashtags == []
            assert response.value_json
...
def _with_entra_enabled():
    return {
        ...
        'entra_auto_provision': settings.entra_auto_provision,
        'entra_default_organisation_slug': settings.entra_default_organisation_slug,
        'entra_default_role': settings.entra_default_role,
        'entra_role_map': settings.entra_role_map,
        'entra_sync_roles': settings.entra_sync_roles,
        'entra_allow_role_elevation': settings.entra_allow_role_elevation,
    }


def _restore_entra_settings(snapshot):
    for name, value in snapshot.items():
        setattr(settings, name, value)


def test_entra_login_route_starts_sign_in(monkeypatch):
    original = _with_entra_enabled()
    try:
        ...
        async def fake_start(_request):
            from fastapi.responses import RedirectResponse
            return RedirectResponse('https://login.microsoftonline.com/mock-auth', 302)

        monkeypatch.setattr('app.main.start_entra_login', fake_start)

        with client:
            response = client.get('/auth/entra/login', follow_redirects=False)
            assert response.status_code == 302
            assert response.headers['location'].startswith('https://login.microsoftonline.com/')
    finally:
        _restore_entra_settings(original)


def test_entra_logout_redirects_to_microsoft_and_clears_session(monkeypatch):
    original = _with_entra_enabled()
    try:
        settings.entra_enabled = True
        settings.entra_tenant_id = 'tenant-id'
        ...
        async def fake_complete(_request):
            return {
                'sub': 'entra-subject',
                'oid': 'entra-oid',
                'preferred_username': 'admin@politis.local',
                'email': 'admin@politis.local',
                'name': 'Platform Owner',
                'tid': 'tenant-id',
            }

        monkeypatch.setattr('app.main.complete_entra_login', fake_complete)

        with client:
            callback = client.get('/auth/entra/callback?state=test', follow_redirects=False)
            assert callback.status_code == 303
            assert callback.headers['location'] == '/'

            logout = post_with_csrf('/logout', follow_redirects=False)
            assert logout.status_code == 303
            assert 'login.microsoftonline.com/tenant-id/oauth2/v2.0/logout' in logout.headers['location']
            assert 'session=""' in logout.headers.get('set-cookie', '')
    finally:
        _restore_entra_settings(original)


def test_entra_callback_handles_invalid_and_expired_tokens(monkeypatch):
    original = _with_entra_enabled()
    try:
        settings.entra_enabled = True
        settings.entra_tenant_id = 'tenant-id'
        ...
        async def invalid_token(_request):
            raise EntraAuthError('Microsoft sign-in token is invalid.')

        monkeypatch.setattr('app.main.complete_entra_login', invalid_token)

        with client:
            invalid = client.get('/auth/entra/callback?state=test', follow_redirects=False)
            assert invalid.status_code == 200
            assert 'Microsoft sign-in token is invalid.' in invalid.text

        async def expired_token(_request):
            raise EntraAuthError('Microsoft sign-in token has expired.')

        monkeypatch.setattr('app.main.complete_entra_login', expired_token)

        with client:
            expired = client.get('/auth/entra/callback?state=test', follow_redirects=False)
            assert expired.status_code == 200
            assert 'Microsoft sign-in token has expired.' in expired.text
    finally:
        _restore_entra_settings(original)


def test_entra_claim_validation_rejects_invalid_tenant():
    original = _with_entra_enabled()
    try:
        settings.entra_enabled = True
        settings.entra_tenant_id = 'tenant-allowed'
        settings.entra_client_id = 'client-id'
        settings.entra_client_secret = 'client-secret'
...
def test_entra_role_mapping_does_not_escalate_existing_user(monkeypatch):
    original = _with_entra_enabled()
    try:
        settings.entra_enabled = True
        settings.entra_tenant_id = 'tenant-id'
        settings.entra_client_id = 'client-id'
        settings.entra_client_secret = 'client-secret'
        settings.entra_sync_roles = True
        settings.entra_allow_role_elevation = False
        settings.entra_role_map = 'Civic.Admin:admin'

        user_email = f"{unique_value('entra-mapped')}@example.org"
        with SessionLocal() as db:
            owner = db.scalar(select(User).where(User.email == 'admin@politis.local'))
            db.add(User(organisation_id=owner.organisation_id, name='Mapped Role User', email=user_email, password_hash=None, role='researcher', external_provider='entra', external_subject='entra-sub'))
            db.commit()
        ...
        with client:
            response = client.get('/auth/entra/callback?state=test', follow_redirects=False)
            assert response.status_code == 303
        ...
        mapped = role_from_claims({'roles': ['Civic.Admin']})
        assert mapped == 'admin'
        assert safe_role_assignment('researcher', mapped) == 'researcher'
    finally:
        _restore_entra_settings(original)
...
def hosted_settings(**overrides):
    data = {
        ...
        'azure_storage_account_url': 'https://storage.example.blob.core.windows.net/',
        'azure_storage_connection_string': '',
        'azure_storage_container': 'evidence',
        'csp_enabled': True,
        'require_redis_rate_limit_in_production': True,
...
def test_stolen_cookie_stops_working_after_admin_password_reset(monkeypatch):
    from app.models import OutboxEmail
    from app.security import hash_password

    researcher_email = f"{unique_value('stolen-cookie')}@example.org"
    researcher_password = 'SecurePass123!'
    reset_token = f"reset-mail-token-{uuid4().hex}"
    monkeypatch.setattr('app.main.new_token', lambda: reset_token)
    ...
    with client:
        ...
        with SessionLocal() as db:
            ...
            assert reset_token not in reset_mail.body
            assert 'token=[REDACTED]' in reset_mail.body
...
def test_password_reset_invalidates_existing_session_cookie(monkeypatch):
    from app.models import OutboxEmail
    from app.security import hash_password, verify_password
    token = f"invalidate-reset-token-{uuid4().hex}"
    monkeypatch.setattr('app.main.new_token', token_generator(token, f"invalidate-session-{uuid4().hex}"))
    ...
    with client:
        ...
        with SessionLocal() as db:
            ...
            assert token not in email_row.body
            assert 'token=[REDACTED]' in email_row.body
