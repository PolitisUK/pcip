...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('Read Participant information (version 7.2)'),
        findsOneWidget,
      );
      await tester.tap(find.text('Read Participant information (version 7.2)'));
      await tester.pumpAndSettle();
      expect(
        find.text('Exact study-specific participant information.'),
        findsOneWidget,
      );
    },
  );
  ...
  test(
    'invitation workflow state takes precedence over participant-wide consent',
    () {
  ...
  testWidgets(
    'bound document load failure is recoverable and cannot submit consent',
    (tester) async {
      var loaded = false;
      ...
      const documents = [
        {
          'document_type': 'participant_information',
          'title': 'Study B information',
          'version': 'B1',
          'reference': 'B-PI',
          'effective_date': '18 August 2026',
          'content_sha256': 'hash-information',
          'body': 'Study B information.',
        },
        {
          'document_type': 'privacy_notice',
          'title': 'Study B privacy',
          'version': 'B1',
          'reference': 'B-PN',
          'effective_date': '18 August 2026',
          'content_sha256': 'hash-privacy',
          'body': 'Study B privacy.',
        },
        {
          'document_type': 'consent_text',
          'title': 'Study B consent',
          'version': 'B1',
          'reference': 'B-CT',
          'effective_date': '18 August 2026',
          'content_sha256': 'hash-consent',
          'body': 'Study B consent.',
        },
      ];
      ...
      await tester.pump();
      expect(
        find.text('Read Study B information (version B1)'),
        findsOneWidget,
      );
      for (final label in [
        'Read Study B information (version B1)',
        'Read Study B privacy (version B1)',
        'Read Study B consent (version B1)',
      ]) {
